/**
 * Character Relationship Utilities
 * 
 * This module provides functions for extracting and interpreting character relationships
 * from the narrative state. It handles relationship scoring, descriptor mapping, and
 * relationship context building for decision generation.
 */
  ...
  importantCharacters.forEach(characterName => {
    ...
    if (narrativeState.narrativeContext?.impactState?.relationshipImpacts) {
      ...
      if (narrativeState.narrativeContext?.impactState?.reputationImpacts &&
        ...
        relationships[characterName] = getRelationshipDescriptor(impact);
      }
