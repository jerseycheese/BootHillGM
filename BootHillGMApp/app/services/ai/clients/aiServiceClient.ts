// BootHillGMApp/app/services/ai/clients/aiServiceClient.ts (Reordered)
...
function processResponse(response: RawApiResponse): DecisionResponse {
  ...
  const metadata = (parsedResponse.metadata as Record<string, unknown>) || { /* Intentionally empty */ };
...
async function makeAPIRequest(
  ...
  while (attempts < config.apiConfig.maxRetries) {
    try {
      ...
      if (!isRetryableError(lastError) || attempts >= config.apiConfig.maxRetries) { // isRetryableError defined above
        break;
      }
...
export async function callAIService(
  ...
  if (rateLimitData.remaining <= 0 && Date.now() < rateLimitData.resetTime) {
    throw createError('RATE_LIMITED', 'Rate limit exceeded', true); // createError defined above
  }

  try {
    // Make the API request
    const response = await makeAPIRequest(prompt, config); // makeAPIRequest defined above

    // Process and return the response
    return processResponse(response); // processResponse defined above
  } catch (error) {
    if (error instanceof Error) {
      throw createError( // createError defined above
        'AI_SERVICE_ERROR',
        error.message,
        isRetryableError(error) // isRetryableError defined above
      );
    }
