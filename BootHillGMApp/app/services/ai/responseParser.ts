// BootHillGMApp/app/services/ai/responseParser.ts (Reordered)
...
function parseStructuredResponse(text: string): AIResponse {
    ...
    try {
      ...
      if (Array.isArray(parsedActions)) {
        parsedActions.forEach(action => {
          // Handle the action format from the test - title instead of text property
          if ((action.title || action.text) &&
              (action.type || 'basic') &&
              ['basic', 'combat', 'interaction'].includes(action.type || 'basic')) {
            suggestedActions.push({
              title: action.title || action.text,
              type: action.type || 'basic',
              description: action.description || '',
              id: action.id || `action-${Date.now()}-${Math.floor(Math.random() * 1000)}`
            } as SuggestedAction);
          }
        });
      }
  ...
  const narrative = processNarrativeText(text); // Defined above

  // Try to extract playerDecision from the response
  const playerDecision = extractPlayerDecision(text, location); // Defined above
...
export function parseAIResponse(text: string): AIResponse | Character {
  ...
  try {
    ...
    const parsedResponse = parseStructuredResponse(text); // Defined above
