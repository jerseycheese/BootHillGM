...
export const InventoryList: React.FC<InventoryListProps> = ({ items, onItemAction }) => {
  ...
  return (
    <ul className="wireframe-list">
      {items.map((item) => (
        <InventoryItem key={item.id} item={item} onAction={(itemId, action) => onItemAction(itemId, action)} />
      ))}
    </ul>
  );
};
